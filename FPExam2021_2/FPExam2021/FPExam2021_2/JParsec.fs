...
module Position =
    ...
    let incrLine p =
        (setLine (getLine p + 1) >> setColumn 0) p


type TextInputState = { lines: string[]; position: Position }
...
module TextInputState =
    ...
    let currentLine is =
        getPosition is |> Position.getLine |> getLine is
    ...
    let getError is =
        ...
        sprintf
            ...
            (lineNumber is)
            (columnNumber is)
            (currentLine is |> Option.get)
            failureCaret

    let incrCol is =
        getPosition is |> Position.incrCol |> setPosition is

    let incrLine is =
        getPosition is |> Position.incrLine |> setPosition is
...
type ParseError =
    ...
    override this.ToString() =
        sprintf "Error parsing %s\n%s%s" this.label (TextInputState.getError this.input) (this.error)
...
module ParseError =
    ...
    let print pe =
        sprintf "Error parsing %s\n%s%s" (getLabel pe) (TextInputState.getError (getInput pe)) (getError pe)
...
module TextParser =
    ...
    let runParser parser input = parser.pfun input
    ...
    let satisfy predicate =
        let parseFun input =
            ...
            | Some sym when predicate sym -> Success(sym, remainingInput)
            | Some sym ->
    ...
    let returnP x =
        mkParser (fun input -> Success(x, input)) "unknown"

    let bindP f p =
        let parseFun input =
            match runParser p input with
            | Failure pe -> Failure pe
            | Success(v, remainingInput) -> runParser (f v) remainingInput
    ...
    let parseZeroOrMore p input =
        let rec aux input acc =
            ...
            | Success(v, remainingInput) -> aux remainingInput (v :: acc)
    ...
    let pchar c =
        satisfy (fun x -> x = c) <?> (sprintf "%c" c)

    let asciiLetter = satisfy Char.IsLetter <?> "letter"

    let anyOf ss =
        ss |> List.map pchar |> choice <?> sprintf "anyOf %A" ss
